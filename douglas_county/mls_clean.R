...
mls <- fread("~/Downloads/Full Export (12).csv", stringsAsFactors = FALSE)
...
# create an sf object that is the MLS number and the Lat, Long columns to then 
# be joined to the doug_sub sf object to get the subdivision for each MLS number (property)
mls_sf <- st_as_sf(mls[, .(`MLS Number`, Latitude, Longitude)], coords = c("Longitude", "Latitude"))

# set coord system
st_crs(mls_sf) <- 4326

# convert to 2232 coord system
mls_sf <- mls_sf %>% st_transform(2232)

# st_join the mls_sf object to the doug_sub object 
mls_sf <- mls_sf %>% st_join(doug_sub, left = TRUE) %>%
  as.data.table()


# setkeys in mls and mls_sf for joining
setkey(mls, "MLS Number")
setkey(mls_sf, "MLS Number")
...
mls <- mls[, .(`MLS Number`, `Last Change Timestamp (System)`, Type, Status, `List Price`,
             ...
             `SqFt Finished`, `SqFt Total`, `SqFt Basement`, `SqFt Main`, `SqFt Total`,
...
names(mls) <- c("mls_number", "last_change_timestamp", "mls_type", "mls_status", "list_price",
               ...
               "sqft_main", "sqft_total", "sqft_upper", "garage_spaces", "carport_spaces",
...
lapply(zips, save_function, mls_dt = mls, file_path = "~/MyStuff/DataScience/RealEstate/douglas_county/mls/")
